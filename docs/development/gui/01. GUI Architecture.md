...
La GUI di My Personal Map è costruita con **CustomTkinter**, un framework moderno per Python che estende Tkinter con un'interfaccia moderna e supporto dark mode nativo.
...
├── backend_manager.py        # FastAPI thread manager
├── config_manager.py         # OS-specific configuration
├── setup_wizard.py           # Database setup wizard
│
├── themes/
│   └── mypersonalmap_theme.json  # CustomTkinter theme
│
├── components/               # Componenti UI riutilizzabili
│   ├── __init__.py
│   ├── custom_button.py     # Bottoni con varianti
│   ├── custom_sidebar.py    # Navigation sidebar
│   └── map_viewer.py        # Folium map embedded
│
├── layouts/                  # Layout completi
│   ├── __init__.py
│   └── main_layout.py       # Layout principale app
│
...
class MapViewer(ctk.CTkFrame):
    ...
    def render(self):
        """Salva HTML → temp file → carica in WebView"""
...
class MyPersonalMapApp(ctk.CTk):
    def __init__(self):
        ...
        # 5. Check DB → Wizard se necessario
        if not self.check_database_configured():
...
└── mypersonalmap.db       # SQLite fallback
```
